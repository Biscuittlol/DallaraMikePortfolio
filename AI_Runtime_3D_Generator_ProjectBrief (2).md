...
![Ford 5.0L Coyote V8 generated at Model fidelity](media/ford_coyote_v8.png)
*A Ford 5.0L Coyote V8 generated from one sentence — 79 named parts, 2,820 triangles, built live in-game at Model fidelity. Left: library thumbnails from other runs; right: the actual output part tree.*
...
![M1 Abrams main battle tank](media/abrams_tank.png)
*"M1 Abrams main battle tank" — road wheels, idlers, sprockets, side skirts, hatches and deck fittings as individually named, selectable parts. Right: the same model in wireframe — real structured geometry, not a mesh shell.*

![Disneyland Ferris wheel](media/disneyland_ferris_wheel.png)
*"Disneyland Ferris wheel" — the system inferred the Mickey-face hub on its own. Sixteen spokes with paired hangers and cabins placed by circular array, over a foundation and loading deck.*
...
![The voxel era](media/history_voxel_era.png)
*Where it started: voxel-based generation. Recognizable volumes — castles, terrain, a chess piece — but monolithic and unlabeled: nothing to select, name, or animate. This is what drove the pivot to part-level blueprints.*

![Early blueprint era](media/history_early_blueprints.png)
*First blueprint era: parts exist, but the spatial contract isn't calibrated yet — engines sink into the ground, gun barrels bury themselves in terrain, proportions drift. Each failure class here maps to a specific rule in the final prompt↔engine contract.*

![Iteration and comparison runs](media/history_naval_iterations.png)
*Comparison runs: fleets, repeated prompts (three lighthouses from one description), and unpainted structure tests. The floating window panes visible here are the placement bug the composite-window contract later eliminated.*

![Generality testing](media/history_buildings.png)
*Generality testing across mundane objects — houses, castles, bus stops, churches, a water tower — where the repeat / array operations earn their keep.*
...
![Chevy small-block V8 — two generations and the part tree](media/chevy_smallblock_v8.png)
*Two independent generations of a Chevy small-block V8 — same prompt, distinct valid designs. The hierarchy panel (right) is the live output: oil pan, flywheel, per-blade fan, spark-wire looms, serpentine belts — every entry an addressable GameObject. Below: the underlying wireframes.*

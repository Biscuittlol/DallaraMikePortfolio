...
- **LLM-agnostic by design.** The model's only responsibility is emitting the blueprint schema; everything vendor-specific lives in one thin, swappable API client, with endpoint and model id as plain configuration. Any LLM capable of structured output can drive the pipeline — the current build integrates Anthropic's Claude family (Opus / Sonnet / Haiku), hot-swappable from an in-game settings panel, and swapping providers means replacing one client class, not the system.
...
- Made the pipeline **vendor-independent**: a provider-agnostic core with a single thin API client boundary, per-request budget overrides (tokens, reasoning effort, timeout), and live in-app model switching.
...
**Languages / Engine:** C#, Unity 6.3. **AI:** LLM-agnostic pipeline — Anthropic Claude API by default (Opus 4.8 with adaptive extended thinking; Sonnet 4.5 / Haiku 4.5 switchable in-app). **3D / Rendering:** ProBuilder (runtime procedural mesh generation), Universal Render Pipeline, TextMeshPro. **Data / Tooling:** Newtonsoft JSON, Unity Test Framework (EditMode), Git.

## Skills demonstrated

LLM application development · AI engineering · prompt engineering · structured / constrained generation · JSON schema · agentic / multi-step LLM pipelines · model-agnostic LLM integration · Anthropic Claude API · extended / adaptive thinking · deterministic systems design · reproducibility · async & concurrent systems · software architecture · separation of concerns · C# · Unity · ProBuilder · runtime procedural generation · computational geometry / 3D math · unit testing (Unity Test Framework) · Git · UI / UX
